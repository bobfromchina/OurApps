...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    private static final String LOG_RESUME_AFTER_PAUSE = ".. Resume view_loading [%s]";
    private static final String LOG_DELAY_BEFORE_LOADING = "Delay %d ms before view_loading...  [%s]";
    private static final String LOG_START_DISPLAY_IMAGE_TASK = "Start display image task [%s]";
    private static final String LOG_WAITING_FOR_IMAGE_LOADED = "Image already is view_loading. Waiting... [%s]";
...
//        decoder = configuration.decoder;
        decoder = getDecoder(engine, imageLoadingInfo);
    ...
    public ImageDecoder getDecoder(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo) {
        if (imageLoadingInfo.options.getImageDecoder() != null) {
            return imageLoadingInfo.options.getImageDecoder();
        } else {
            return engine.configuration.decoder;
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (bmp == null || bmp.isRecycled()) {
                bmp = tryLoadBitmap();
                if (bmp == null) return; // listeneLinearLayoutr callback already was fired
    ...
    /**
     * @return <b>true</b> - if view_loading should be continued; <b>false</b> - if view_loading should be interrupted
     */
