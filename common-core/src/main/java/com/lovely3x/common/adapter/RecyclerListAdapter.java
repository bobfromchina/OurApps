 ...
 * @param <T>
 */
public abstract class RecyclerListAdapter<T> extends RecyclerView.Adapter<BaseRecyclerViewHolder> {
     ...
     * 显示参数
     */
    protected final ImageDisplayOptions options = CommonApplication.getInstance().getImageLoaderDisplayOptionsBuilder().build();
    ...
    protected TextWatchedListener mTextWatchedListener;
     ...
     * @param context 上下文对象
     */
    public RecyclerListAdapter(List<T> datas, Context context) {
    ...
    public List<String> getCheckItems() {
        ...
        for (Map.Entry<String, Boolean> entry : entrySet) {
            if (entry.getValue() != null && entry.getValue()) checkedItems.add(entry.getKey());
        }
     ...
     * @param context 上下文对象
     */
    public RecyclerListAdapter(final T[] datas, Context context) {
                ...
                public void onClick(View v) {
                    ...
                    if (mOnItemClickedListener != null) {
                        ...
                        if (recyclerView instanceof RecyclerViewHeaderAndFooter) {
                            int headerCount = ((RecyclerViewHeaderAndFooter) recyclerView).getHeaderCount();
                            mOnItemClickedListener.onClicked(pos, getItem(pos - headerCount));
                        } else {
                            mOnItemClickedListener.onClicked(pos, getItem(pos));
                        }
                    }
                }
            });
     ...
     *
     * @param type   需要创建的视图的类型
     * @param parent parent 咯
     * @return 创建的viewHolder
     */
    public abstract
    @NonNull
    BaseRecyclerViewHolder createViewHolder(int type, ViewGroup parent);
     ...
     * @param holder   holder
     */
    public abstract void handleData(int position, @NonNull BaseRecyclerViewHolder holder);
    ...
    public void setOnViewTextWatchedListener(TextWatchedListener listener){
        this.mTextWatchedListener = listener;
    }
// TODO: 2017/6/29
    /**
     * 根据标识删除元素
     * 注意的是,必须是当前adapter的数据源实现了{@link Identity 接口才行}
     *
     * @param id
     */
    public boolean deleteById(Identity id) {
        if (id == null || datas == null) return false;
        for (T t : datas) {
            if (t instanceof Identity)
                if (((Identity) t).getUniqueID().equals(id.getUniqueID())) {
                    datas.remove(t);
                    notifyDataSetChanged();
                    return true;
                }
    ...
    public boolean deleteById(String id) {
        ...
        for (T t : datas) {
            if (t instanceof Identity)
                if (((Identity) t).getUniqueID().equals(id)) {
                    datas.remove(t);
                    notifyDataSetChanged();
                    return true;
                }
     ...
     * @param id 需要查找的id
     */
    public T getElementById(Identity id) {
        if (id == null || datas == null || id.getUniqueID() == null) return null;
        for (T t : datas) {
            if (t instanceof Identity)
                if (id.getUniqueID().equals(((Identity) t).getUniqueID())) {
                    return t;
                }
        }
        return null;
    ...
    public T findItemById(String id) {
        ...
        for (T t : datas) {
            if (t instanceof Identity) {
                if (id.equals(((Identity) t).getUniqueID())) {
                    return t;
                }
            }
        }
    ...
    public T findItemById(Identity id) {
        if (id == null) throw new NullPointerException();
        for (T t : datas) {
            if (t instanceof Identity) {
                if (id.getUniqueID().equals(((Identity) t).getUniqueID())) {
    ...
    public interface OnCheckedItemChangedListener {
         ...
         */
        void onCheckedItemChanged(RecyclerListAdapter tListAdapter, int checkedItemCount, String changedItemId, boolean isChecked);
     ...
     * 视图点击监听器
     */
    public interface TextWatchedListener {
         ...
         * @param view     被点击的视图对象
         */
        void onViewTextWatched(int position, View view);
    }
}
