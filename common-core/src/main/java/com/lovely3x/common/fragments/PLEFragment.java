 ...
 */
public abstract class PLEFragment extends ExactEmptyContentTipFragment implements RefreshAndLoadMore, PtrHandler, LoadMoreHandler {
    ...
    protected void initLoadMoreLayoutOptions() {
        ...
        if (mLoadMoreLayout.getChildCount() > 0 && mLoadMoreLayout.getChildAt(0) instanceof RecyclerView) {
            ...
            if (adapter instanceof AdvancedAdapter2) {
//                ((LoadMoreRecycleViewContainer) mLoadMoreLayout).setCubeRecyclerViewAdapter((AdvancedAdapter2) adapter);
            }
        }
    ...
    public void autoRefresh() {
        if (mRefreshLayout != null && canRefresh) {
            ...
            if (mRefreshLayout.getHeight() > 0) getHandler().post(run);
            else mRefreshLayout.postDelayed(run, 100);
    ...
    public boolean checkCanDoRefresh(PtrFrameLayout frame, View content, View header) {
        if (content instanceof LoadMoreContainerBase) {
            return !PtrDefaultHandler.canChildScrollUp(((LoadMoreContainerBase) content).getChildAt(0));
        } else {
     ...
     * @param delay
     */
    public void refreshComplete(long delay) {
