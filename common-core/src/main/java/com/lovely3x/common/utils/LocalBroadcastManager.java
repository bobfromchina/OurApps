...
public class LocalBroadcastManager {
    ...
    private final List<BroadcastReceiver> mAbortBroadcasts = new Vector<>();
    ...
    private void executePendingBroadcasts() {
        while (true) {
            ...
            for (BroadcastRecord br : brs) {
                //按照优先级排序
                Collections.sort(br.receivers, new Comparator<ReceiverRecord>() {
                    @Override
                    public int compare(ReceiverRecord lhs, ReceiverRecord rhs) {
                        return rhs.filter.getPriority() - lhs.filter.getPriority();
                    }
                });

                for (int j = 0; j < br.receivers.size(); j++) {
                    BroadcastReceiver receiver = br.receivers.get(j).receiver;
                    receiver.onReceive(mAppContext, br.intent);
                    synchronized (mAbortBroadcasts) {
                        if (mAbortBroadcasts.contains(receiver)) {
                            mAbortBroadcasts.remove(receiver);
                            break;
                        }
                    }
                }
    ...
    public void abortBroadcast(BroadcastReceiver receiver) {
        synchronized (mAbortBroadcasts) {
            this.mAbortBroadcasts.add(receiver);
        }
    }
